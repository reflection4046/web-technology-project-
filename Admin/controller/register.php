 ...
 $nameErr = $emailErr = $dobErr = $genderErr = $unameErr = $passErr = $confirmpassErr = "";
 $name = $email = $uname = $dd = $mm = $yyyy = $gender = $password = $confirmpassword = "";
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["uname"])) {
      ...
      $uname = $_POST["uname"];
      $count = strlen("$uname");
      if ((!preg_match("/^[a-zA-Z-_' ]*$/",$uname)) || $count < 1 ){
    ...
    if (empty($_POST["password"])) {
      ...
      if ((!preg_match("([@#$%])",$password)) || $count <  4) {
